...
export default async function(req) {
  try {
    ...
    const recipients = broadcast ? [...new Set((await base44.asServiceRole.entities.CustomerProfile.list('-created_date', 500)).map(profile => profile.user_email).filter(Boolean))] : [customer_email].filter(Boolean);
