...
export default function ManualNotificationForm() {
  const { can, canFull } = useAdminPermissions();
  ...
  useEffect(() => { base44.entities.CustomerProfile.filter({ is_archived: false }, '-created_date', 500).then(setProfiles).catch(() => {}); }, []);
